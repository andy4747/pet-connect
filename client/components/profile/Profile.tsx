...
import { GET_PROFILE_DATA } from '../../graphql/queries/getProfileData';
			...
			<div className='mx-auto'>
				<div className='px-3 py-2'>
					<div className='flex flex-col gap-1 text-center'>
						...
						<p className='font-serif font-semibold'>{data.me.username}</p>
					</div>

					<div className='flex justify-center items-center gap-2 my-3'>
						<div className='font-semibold text-center mx-4'>
							<p className='text-black'>{data.getProfileData.posts.length}</p>
							<span className='text-gray-400'>Posts</span>
						</div>
						<div className='font-semibold text-center mx-4'>
							<p className='text-black'>
								{data.getProfileData.followers.length}
							</p>
							<span className='text-gray-400'>Followers</span>
						</div>
						<div className='font-semibold text-center mx-4'>
							<p className='text-black'>
								{data.getProfileData.following.length}
							</p>
					...
					<div className='grid grid-cols-3 gap-4 my-3 ml-3'>
						{data.getProfileData.posts.map((item: any) => {
							return (
								<div
									key={item.id}
