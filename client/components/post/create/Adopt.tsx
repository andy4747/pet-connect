...
export const Adopt = () => {
	...
	const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		const adoptionData = new FormData();
		adoptionData.append('post-image', picture!);
		adoptionData.append('title', title);
		adoptionData.append('body', body);
		adoptionData.append('species', species);
		adoptionData.append('color', color);
		adoptionData.append('age', String(age));

		//posting data to the server
		const response = await fetch('http://localhost:5000/post-adopt', {
			body: adoptionData,
			method: 'post',
			credentials: 'include',
		});
