...
export const Donate = () => {
	...
	const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
		...
		donationData.append('post-image', picture!);
		donationData.append('title', title);
		donationData.append('body', body);
		donationData.append('species', species);
		donationData.append('color', color);
		donationData.append('age', String(age));

		//posting data to the server
		const response = await fetch('http://localhost:5000/post-donate', {
			body: donationData,
			method: 'post',
			credentials: 'include',
		});
